...
#: The class of the export every map actor hangs off. This, not a path prefix, is what
#: makes an export an actor -- see the module docstring.
        ...
        for block in range(first, last + 1):
            ...
            if entry[11] == 0:
                ...
                if len(chunk) != unpacked_size:
                    raise ParseError(f"block {block}: got {len(chunk)}, wanted {unpacked_size}")
                out += chunk
            self.blocks_read += 1
...
# --------------------------------------------------------------------------------------
# The packages: a zen (cooked, IoStore-resident) .umap header and its property tags.
# --------------------------------------------------------------------------------------
...
class ScriptObjects:
    ...
    def __init__(self, paks: Path, decompress) -> None:
        store = IoStore(paks, "global", decompress)
        chunks = store.chunks_of_type(self.CHUNK_TYPE)
        if not chunks:
            raise ParseError("global.utoc holds no ScriptObjects chunk")
...
class AssetIndex:
    ...
    Every class-side fact in this file -- a component template, a creature's
...
def main() -> int:
    ...
    scripts = ScriptObjects(paks, ooz.decompress)
    print(
    ...
    store = IoStore(paks, "FactoryGame-Windows", ooz.decompress)
    print(
