...
#: The loaded table, keyed by the file and its mtime -- the same key ``spatial.heightfield``
#: uses, and for the same reason. This was an ``lru_cache(maxsize=1)``, which is a cache on
#: NOTHING: the artifact is regenerated by ``tools/gen_resource_nodes.py`` on the reader's own
#: machine, and every answer for the rest of the server's life came from the table read before
#: the regeneration. That is the worst way to be stale, because the generator prints its new
#: counts and the server goes on quoting the old ones. Keyed on the mtime it is picked up on
#: the next call, with no restart and no cache-clearing call anybody has to remember.
#:
#: One entry, like the ``maxsize=1`` it replaces: the previous table is dead the moment a
#: newer one is read, so the miss clears rather than accumulating a table per regeneration.
