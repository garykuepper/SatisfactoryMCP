...
def _classify(
    key: str, record: dict, game: GameData, unwired: bool, short: str, conduits, heads=None
):
...
def assess(
    ...
    def far_health(actor: str) -> tuple[dict | None, str]:
        ...
        return record, _classify(key, record, game, dark, actor, conduits, heads)[0]

    for short, (key, record) in everything.items():
        if short not in wanted:
            continue
        state, cause, uptime, recipe = _classify(
            key, record, game, short in unwired, short, conduits, heads
        )
        entry = MachineHealth(
            instance=short,
