 ...
 *
 * This file is a MODULE now, and the two consequences are worth stating because the page
 * used to be a classic script. Nothing declared here is a global any more -- `map` and
 * `state` were reachable from the browser console before and are not now -- and the bundle
 * loads with `type="module"`, so it is deferred and runs after the document is parsed rather
 * than from the bottom of <body>. Every element it reaches for exists under both
 * arrangements, which is why the port is a move. `"use strict"` is gone for the same reason
 * and not as a relaxation: a module is strict already, and the directive is ignored in one.
 */
