...
def test_excluded_recipes_leave_the_scenario(game, state):
    req = build_scenario(game, state, sources=["region:Spire Coast"], exclude_recipes=["Recycled"])
...
def test_a_ban_that_matches_nothing_is_reported_never_ignored(game, state):
    ...
    req = build_scenario(game, state, sources=["region:Spire Coast"], exclude_recipes=["Nope"])
...
def test_an_exact_name_bans_only_that_recipe(game, state):
    ...
    req = build_scenario(game, state, sources=["region:Spire Coast"], exclude_recipes=["Plastic"])
...
def test_only_recipes_restricts_to_the_named_set(game, state):
    req = build_scenario(
        game, state, sources=["region:Spire Coast"], only_recipes=["Heavy Oil Residue", "Diluted"]
    )
...
def test_plan_id_changes_when_a_recipe_is_banned(game, state):
    ...
    b = build_scenario(game, state, sources=["region:Spire Coast"], exclude_recipes=["Recycled"])
    assert a.plan_id != b.plan_id


def test_plan_id_is_stable_for_identical_arguments(game, state):
    a = build_scenario(game, state, sources=["region:Spire Coast"])
    b = build_scenario(game, state, sources=["region:Spire Coast"])
    assert a.plan_id == b.plan_id
