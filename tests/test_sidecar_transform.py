"""Schema 12: the placement yaw and the belt splines the projection used to drop.

Both are geometry the parser already decoded and the projection threw away, and both are here
for the same reason -- a client could only draw the world axis-aligned and beltless, so an
angled platform came out as a staircase and a factory came out as a scatter of rectangles.
...
off the wrong axis, is still a plausible float on every record. A belt spline left in the chain
actor's own frame is still a plausible polyline -- 1,500 m from where the belt is. So the tests
below check the numbers against *other* numbers in the same save: yaw against the direction
neighbouring foundations are actually laid out in, belt points against where the buildings are.
Each one fails on the mistake it is named for.
...
from satisfactory_mcp.core.saveio.extract import _belts, _conveyor_class, yaw_of
