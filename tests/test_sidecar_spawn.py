...
def test_a_schema_bump_makes_every_cached_projection_miss(monkeypatch):
    ...
    to include the shape it was written in. Without that, schema 18 would hand out a schema-17
    pickle -- one with no ``crates`` key, so a world where the player died in the middle of
    nowhere carrying half a factory would report nothing lying on the ground at all -- as 17
    would have handed out a 16 with no ``power`` key and a map that draws wires drawing none,
    and 16 a 15 that buckets eight containers' contents as unspendable machine buffers and
    calls an unreadable rotation axis-aligned.
    ...
    assert proj._cache_key(header) != now, "a schema 17 pickle would be served to schema 18"

    monkeypatch.setattr(proj, "SCHEMA_VERSION", 18)
